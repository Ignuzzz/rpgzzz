...
#  - botão para deletar ficha (com deleção em cascata das entidades relacionadas)
...
st.title("RPG com botão de download do DB")

# --- Exemplo de alteração ---
if st.button("Adicionar item de teste"):
    c.execute("CREATE TABLE IF NOT EXISTS itens (nome TEXT)")
    c.execute("INSERT INTO itens (nome) VALUES (?)", ("Espada Mágica",))
    conn.commit()
    st.success("Item adicionado no DB da sessão!")

# --- Botão de download do DB ---
with open(DB_PATH, "rb") as f:
    db_bytes = f.read()

st.download_button(
    label="Baixar rpg.db atual",
    data=db_bytes,
    file_name="rpg.db",
...
def get_conn():
    ...
    # garante que ON DELETE CASCADE funciona em TODAS as conexões
...
def delete_character_if_allowed(cid: int, user: dict) -> bool:
    """
    Deleta a ficha informada se o usuário tiver permissão:
      - admin pode deletar qualquer ficha
      - usuário comum só pode deletar fichas em que é o owner
    Graças aos FKs com ON DELETE CASCADE, isso apaga também stats, combat,
    inventário, habilidades da ficha, aparências, etc.
    ...
    with get_conn() as conn, conn:
        ...
        # agora usamos rowcount do CURSOR, não da conexão
...
def main():
    ...
    if st.session_state.get(refresh_key):
        ...
        try:
            ...
            # 🔄 BOTÃO DE ATUALIZAR FICHA (mantém ficha e aba atuais)
    st.markdown("### Ações da ficha")
    if st.button("🔄 Atualizar ficha atual", key=f"refresh_char_{cid}"):
        st.rerun()
    ...
    # ===== BLOCO PARA DELETAR A FICHA ATUAL =====
    # Só mostra se o usuário é admin ou dono da ficha
    can_delete = bool(user.get("is_admin") or (ch and ch["owner_user_id"] == user["id"]))
    if can_delete:
        st.markdown("---")
        with st.expander("🗑️ Apagar esta ficha", expanded=False):
            st.warning(
                "Essa ação apagará **esta ficha e todos os dados ligados a ela** "
                "(status, combate, inventário, habilidades da ficha, aparências, anotações, etc.). "
                "Não é possível desfazer."
            )
            col_del_1, col_del_2 = st.columns([1, 3])
            with col_del_1:
                if st.button("Confirmar exclusão desta ficha", key=f"confirm_del_char_{cid}"):
                    ok = delete_character_if_allowed(cid, user)
                    if ok:
                        st.success("Ficha apagada com sucesso.")
                        # Decide próxima ficha (se existir) ou limpa o estado
                        chars = list_characters(user)
                        if chars:
                            new_cid = chars[0]["id"]
                            st.session_state.cid = new_cid
                            params = dict(st.query_params)
                            params["char"] = [str(new_cid)]
                            st.query_params = params
                        else:
                        ...
                        st.error("Não foi possível apagar a ficha (sem permissão ou já removida).")
            with col_del_2:
                st.caption("Use com cuidado. Recomende tirar print ou backup antes de excluir definitivamente.")
